...
class LevelDesigner extends FlameBlocGame
  ...
  late DesignerGameState gameState;
  ...
  Future<void> onLoad() async {
    ...
    if (iModel != null) {
      gameState.levelName = iModel!.LevelName;
      gameState.baseMatrix = iModel!.Layers;
    }
  ...
  void onAttach() {
    ...
    if (iModel != null) {
      ...
      mUICubit.setBackgroundGradientBeginColor(
          Color(int.parse(iModel!.hexGradientStart, radix: 16)));
      mUICubit.setBackgroundGradientEndColor(
          Color(int.parse(iModel!.hexGradientEnd, radix: 16)));
      mUIState = mUICubit.state;
    }
    assignBackground(
        startingColor: mUIState.backgroundBeginColor,
        endingColor: mUIState.backgroundEndColor);
  }

  void computeEnvironment(int initial) {
    for (int i = initial; i < gameState.baseMatrix.length; i++) {
      final newPosition = Vector2(0, -1 * i * tileHeight / 2);
      envLayers.add(IsometricTileMapCustom(
        tileset,
        gameState.baseMatrix[i],
        priority: i,
        angle: 0,
  ...
  void moveGridForLayer(bool up) {
    ...
    up
        ? _grid.position.sub(Vector2(0, tileHeight / 2))
        : _grid.position.add(Vector2(0, tileHeight / 2));
    up
        ? _highlight.position.sub(Vector2(0, tileHeight / 2))
        : _highlight.position.add(Vector2(0, tileHeight / 2));
  }

  void saveLevel(
      {required bool includeTileSet,
  ...
  void render(Canvas canvas) {
    if (reRenderBackground) {
      ...
      assignBackground(
          startingColor: state.backgroundBeginColor,
          endingColor: state.backgroundEndColor);
    ...
    if (debugMode) {
      ...
      if (currentfps < 20) {
        fpsTextPaint.render(
          canvas,
          currentfps.roundToDouble().toString(),
          Vector2(0, 200),
        );
      }
  ...
  void onTapUp(TapUpInfo info) {
    ...
    if (currentUIState is LevelGenUILoaded) {
      final int totalLayersBefore = gameState.baseMatrix.length;
      bool hasEnvChanged = gameState.checkForLayer(currentUIState.currentLayer);
      if (hasEnvChanged) {
        computeEnvironment(totalLayersBefore);
      }

      final block =
          envLayers[currentUIState.currentLayer].getBlock(screenPosition);
      gameState.toggleIndex(Vector2Int(x: block.x, y: block.y),
          currentUIState.lastTileIndex, currentUIState.currentLayer);
  ...
  void onScaleStart(ScaleStartInfo info) {
    ...
    if (_clickStart != null &&
        currentTime.difference(_clickStart!).inMilliseconds > 600) {
      _clickHeld = true;
    }
  }
  ...
  void onScaleEnd(ScaleEndInfo info) {
    ...
    if (_clickHeld) {
      ...
      if (currentUIState is LevelGenUILoaded) {
        final int totalLayersBefore = gameState.baseMatrix.length;
        bool hasEnvChanged =
            gameState.checkForLayer(currentUIState.currentLayer);
        if (hasEnvChanged) {
          computeEnvironment(totalLayersBefore);
        }
        gameState.toggleIndexRangeForLastHighlight(
            replaceIndex: currentUIState.lastTileIndex,
            layerIndex: currentUIState.currentLayer);
      }
      gameState.resetHighlight();
      _highlight.matrix = gameState.highLightMatrix;
    }
    _clickHeld = false;
  }
  ...
  void onScaleUpdate(ScaleUpdateInfo info) {
    ...
    if (!currentScale.isIdentity()) {
      ...
      if (_clickHeld) {
        ...
        final screenPosition = info.eventPosition.game;
        final block = envLayers[currentUICubit.state.currentLayer]
            .getBlock(screenPosition);
        if (block.x >= 0 &&
            block.x <= gameState.size &&
            block.y >= 0 &&
            block.y <= gameState.size) {
          // is inside grid bounds
          final Block startBlock = envLayers[currentUICubit.state.currentLayer]
              .getBlock(startPosition);
          gameState.highlight(Vector2Int.fromBlock(block: startBlock),
              Vector2Int.fromBlock(block: block));
        } else {
